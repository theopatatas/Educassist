      ...
      {selected ? (
        <div className="fixed inset-0 z-[70] flex justify-end bg-slate-950/45" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}>
          <aside className="h-full w-full max-w-2xl overflow-y-auto bg-white p-5 shadow-2xl sm:p-6">
            ...
            <section className="mt-4 rounded-2xl border p-4"><h3 className="flex items-center gap-2 font-bold"><History className="h-4 w-4" />Takeover Activity</h3><div className="mt-3 space-y-2">{activitiesLoading ? <p className="text-sm text-slate-500">Loading activities…</p> : activities.length ? activities.map((item) => <div key={item.id} className="rounded-xl bg-slate-50 p-3"><p className="text-sm font-medium">{label(item.action)}</p><p className="mt-1 text-xs text-slate-500">{item.details || "Activity recorded"}{item.affectedClass ? ` · ${item.affectedClass}` : ""}{item.user?.name ? ` · ${item.user.name}` : ""} · {new Date(item.createdAt).toLocaleString()}</p></div>) : <p className="text-sm text-slate-500">No takeover activities have been recorded.</p>}</div></section>
          </aside>
        </div>
      ) : null}
    </div>
