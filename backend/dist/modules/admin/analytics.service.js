...
exports.analyticsExportRows = analyticsExportRows;
...
const ChatLog_model_1 = require("../../db/models/ChatLog.model");
...
const dateValue = (model, key) => model.getDataValue(key);
const monthLabel = (value) => value.toLocaleDateString("en-US", { month: "short", year: "numeric" });
const dateWhere = (from, to) => from || to
    ? {
        ...(from ? { [sequelize_1.Op.gte]: new Date(`${from}T00:00:00`) } : {}),
        ...(to ? { [sequelize_1.Op.lte]: new Date(`${to}T23:59:59`) } : {}),
    }
    : undefined;
...
async function getAdminAnalytics(filters) {
    ...
    const [academic, sections, subjects, teachers, students, classes, parents, parentLinks, users, leaves, takeovers, auditLogs, chatLogs, lessonPlans,] = await Promise.all([
        ...
        ClassTakeover_model_1.ClassTakeover.findAll(),
        SystemAuditLog_model_1.SystemAuditLog.findAll({ order: [["createdAt", "DESC"]], limit: 5000 }),
        ChatLog_model_1.ChatLog.findAll(),
        LessonPlan_model_1.LessonPlan.findAll(),
    ]);
    const sectionMap = new Map(sections.map((row) => [number(row.id), row]));
    const selectedSection = sections.find((row) => String(row.id) === filters.section ||
        row.name.toLowerCase() === String(filters.section ?? "").toLowerCase());
    const selectedSubject = subjects.find((row) => String(row.id) === filters.subject ||
        row.name.toLowerCase() === String(filters.subject ?? "").toLowerCase());
    const classScope = classes.filter((row) => (!filters.schoolYear ||
    ...
    const hasClassFilter = Boolean(filters.schoolYear ||
        filters.gradeLevel ||
        filters.section ||
        filters.subject ||
        filters.teacher);
    const classIds = new Set(classScope.map((row) => number(row.id)));
    const studentScope = students.filter((row) => (!filters.student || String(row.id) === filters.student) &&
        (!filters.gradeLevel || row.yearLevel === filters.gradeLevel) &&
        (!filters.section || number(row.sectionId) === number(selectedSection?.id)));
    const hasStudentFilter = Boolean(filters.student || filters.gradeLevel || filters.section);
    ...
    if (enrollmentDates)
        enrollmentWhere.enrolledAt = enrollmentDates;
    const [enrollments, attendance, gradeItems, assignments, quizzes] = await Promise.all([
        Enrollment_model_1.Enrollment.findAll({ where: enrollmentWhere }),
        ...
        Assignment_model_1.Assignment.findAll({
            where: {
    ...
    const scopedEnrollments = enrollments.filter((row) => (!hasClassFilter || classIds.has(number(row.classId))) &&
        (!hasStudentFilter || studentIds.has(number(row.studentId))));
    const scopedAttendance = attendance.filter((row) => (!hasClassFilter || classIds.has(number(row.classId))) &&
        (!hasStudentFilter || studentIds.has(number(row.studentId))));
    const scopedGradeItems = gradeItems.filter((row) => (!hasClassFilter || classIds.has(number(row.classId))) &&
    ...
    const grades = gradeItemIds.length
        ? await Grade_model_1.Grade.findAll({ where: { gradeItemId: gradeItemIds } })
        : [];
    const scopedGrades = grades.filter((row) => !hasStudentFilter || studentIds.has(number(row.studentId)));
    const scopedAssignments = assignments.filter((row) => row.classId && (!hasClassFilter || classIds.has(number(row.classId))));
    const assignmentIds = scopedAssignments.map((row) => number(row.id));
    const assignmentSubmissions = assignmentIds.length
        ? await AssignmentSubmission_model_1.AssignmentSubmission.findAll({
            where: { assignmentId: assignmentIds },
        })
        : [];
    const scopedQuizzes = quizzes.filter((row) => row.classId && (!hasClassFilter || classIds.has(number(row.classId))));
    ...
    const quizAttempts = quizIds.length
        ...
        : [];
    const enrollmentMonths = countBy(scopedEnrollments, (row) => monthLabel(row.enrolledAt || dateValue(row, "createdAt") || new Date()));
    const attendanceStatus = countBy(scopedAttendance, (row) => row.status.toLowerCase());
    ...
    const attendanceRate = attendanceTotal
        ...
        : 0;
    const attendanceTrend = (format) => {
        const groups = new Map();
        scopedAttendance.forEach((row) => {
            const date = new Date(`${row.date}T00:00:00`);
            const label = format === "daily"
                ? row.date
                : format === "monthly"
                    ? monthLabel(date)
    ...
    const gradeLevelValues = new Map();
    ...
    const sortedGradeLevels = [...gradeLevelAverages].sort((left, right) => right.value - left.value);
    const subjectRows = subjects
        .map((subject) => {
        const matchingClasses = classScope.filter((row) => number(row.subjectId) === number(subject.id));
        const values = matchingClasses.flatMap((row) => gradesByClass.get(number(row.id)) ?? []);
        const enrolled = new Set(scopedEnrollments
            .filter((row) => matchingClasses.some((item) => number(item.id) === number(row.classId)))
    ...
    const rankedSubjects = subjectRows
        ...
        .sort((left, right) => number(right.averageGrade) - number(left.averageGrade));
    const sectionRows = sections
        .map((section) => {
        const matchingStudents = studentScope.filter((row) => number(row.sectionId) === number(section.id));
        const matchingClasses = classScope.filter((row) => number(row.sectionId) === number(section.id));
        const sectionStudentIds = new Set(matchingStudents.map((row) => number(row.id)));
        const gradeValues = studentAverages
            .filter((row) => sectionStudentIds.has(row.studentId))
            .map((row) => row.average);
        const attendanceRows = scopedAttendance.filter((row) => sectionStudentIds.has(number(row.studentId)));
        const adviser = teachers.find((row) => number(row.sectionId) === number(section.id));
        return {
            ...
            attendanceRate: attendanceRows.length
                ? round((attendanceRows.filter((row) => row.status !== "absent").length /
    ...
    const gradeComparison = [...new Set(studentScope.map((row) => row.yearLevel || "Unassigned"))].map((gradeLevel) => {
        const matching = studentScope.filter((row) => (row.yearLevel || "Unassigned") === gradeLevel);
        const ids = new Set(matching.map((row) => number(row.id)));
        const gradeRows = studentAverages.filter((row) => ids.has(row.studentId));
        const attendanceRows = scopedAttendance.filter((row) => ids.has(number(row.studentId)));
        return {
            gradeLevel,
            ...
            passingRate: gradeRows.length
                ? round((gradeRows.filter((row) => row.average >= 75).length /
                    gradeRows.length) *
                    100)
                : null,
        };
    ...
    const teacherWorkload = teachers
        ...
        .map((teacher) => {
        const assigned = classScope.filter((row) => number(row.teacherId) === number(teacher.id));
        const enrolled = new Set(scopedEnrollments
        ...
        return {
            ...
            averageGrade: assigned.flatMap((row) => gradesByClass.get(number(row.id)) ?? []).length
                ? round(average(assigned.flatMap((row) => gradesByClass.get(number(row.id)) ?? [])) ?? 0)
                : null,
        };
    });
    const workloadValues = teacherWorkload.map((row) => row.students);
    const assignmentSubmitted = assignmentSubmissions.filter((row) => row.submittedAt &&
        (!hasStudentFilter || studentIds.has(number(row.studentId)))).length;
    const assignmentExpected = scopedAssignments.reduce((sum, assignment) => sum +
        new Set(scopedEnrollments
            .filter((row) => number(row.classId) === number(assignment.classId))
            .map((row) => row.studentId)).size, 0);
    const completedQuizAttempts = quizAttempts.filter((row) => row.completedAt &&
        (!hasStudentFilter || studentIds.has(number(row.studentId))));
    ...
    const scopedUsers = filters.dateFrom || filters.dateTo
        ? users.filter((row) => withinDateRange(dateValue(row, "createdAt"), filters.dateFrom, filters.dateTo))
        : users;
    const scopedLeaves = filters.dateFrom || filters.dateTo
        ? leaves.filter((row) => withinDateRange(dateValue(row, "createdAt"), filters.dateFrom, filters.dateTo))
        : leaves;
    const scopedTakeovers = filters.dateFrom || filters.dateTo
        ? takeovers.filter((row) => withinDateRange(dateValue(row, "createdAt"), filters.dateFrom, filters.dateTo))
        : takeovers;
    const scopedAuditLogs = filters.dateFrom || filters.dateTo
        ? auditLogs.filter((row) => withinDateRange(row.createdAt, filters.dateFrom, filters.dateTo))
        : auditLogs;
    const scopedChatLogs = chatLogs.filter((row) => (!filters.teacher ||
        String(row.userId) ===
            String(teachers.find((teacher) => String(teacher.id) === filters.teacher)
                ?.userId ?? "")) &&
        (!(filters.dateFrom || filters.dateTo) ||
            withinDateRange(dateValue(row, "createdAt"), filters.dateFrom, filters.dateTo)));
    const scopedLessonPlans = lessonPlans.filter((row) => (!filters.teacher || String(row.teacherId) === filters.teacher) &&
        (!filters.gradeLevel || row.gradeLevel === filters.gradeLevel) &&
        (!filters.subject ||
    ...
    const registrationMonths = countBy(scopedUsers, (row) => monthLabel(dateValue(row, "createdAt") || new Date()));
    const activityDays = countBy(scopedAuditLogs, (row) => row.createdAt.toISOString().slice(0, 10));
    ...
    const newStudents = studentScope.filter((row) => filters.dateFrom || filters.dateTo
        ? withinDateRange(dateValue(row, "createdAt"), filters.dateFrom, filters.dateTo)
        : String(dateValue(row, "createdAt")?.toISOString() ?? "").startsWith(currentMonth)).length;
    const previousMonthDate = new Date();
    previousMonthDate.setMonth(previousMonthDate.getMonth() - 1);
    const previousMonth = previousMonthDate.toISOString().slice(0, 7);
    const previousNewStudents = students.filter((row) => String(dateValue(row, "createdAt")?.toISOString() ?? "").startsWith(previousMonth)).length;
    const enrollmentGrowth = previousNewStudents
        ? round(((newStudents - previousNewStudents) / previousNewStudents) * 100)
        : null;
    const incompleteProfiles = studentScope.filter((row) => !row.gender ||
        !row.studentMobileNumber ||
        !row.guardianName ||
        !row.guardianContact).length;
    return {
        ...
        aiUsage: {
            ...
            teacherConversations: scopedChatLogs.filter((row) => row.role.toLowerCase() === "teacher").length,
            lessonPlans: scopedLessonPlans.length,
            finalizedLessonPlans: scopedLessonPlans.filter((row) => row.status === "final").length,
        },
        parents: {
            total: parents.length,
            linkedStudents: studentScope.filter((row) => linkedStudentIds.has(number(row.id))).length,
            coverageRate: studentScope.length
                ? round((studentScope.filter((row) => linkedStudentIds.has(number(row.id))).length /
                    studentScope.length) *
                    100)
                : null,
        ...
        insights: [
            ...
            ...(gradeComparison.length
                ? [{
                        ...
                        value: [...gradeComparison].sort((left, right) => number(right.attendanceRate) - number(left.attendanceRate))[0]?.gradeLevel,
                    }]
                : []),
        ],
        dataQuality: [
...
function analyticsExportRows(analytics) {
    return [
        ["Metric", "Value"],
