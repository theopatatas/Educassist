...
function auditPresentation(row: SystemAuditLog) {
  const metadata =
...
export async function getRecentActivities() {
  const [
    ...
    SystemAuditLog.findAll({
      order: [["createdAt", "DESC"]],
      limit: 30,
    }),
    AdminAccountActivity.findAll({
      order: [["createdAt", "DESC"]],
      limit: 20,
    }),
    User.findAll({ order: [["createdAt", "DESC"]], limit: 20 }),
    Student.findAll({ order: [["createdAt", "DESC"]], limit: 15 }),
    ParentStudent.findAll({ order: [["createdAt", "DESC"]], limit: 15 }),
    Class.findAll({ order: [["createdAt", "DESC"]], limit: 15 }),
    GradeItem.findAll({
      where: { name: { [Op.like]: "%|published" } },
      order: [["updatedAt", "DESC"]],
      limit: 20,
    }),
    SchoolEvent.findAll({ order: [["createdAt", "DESC"]], limit: 15 }),
  ]);
  const actorIds = new Set<number>();
  auditRows.forEach((row) => actorIds.add(Number(row.userId)));
  accountRows.forEach((row) => actorIds.add(Number(row.actorUserId)));
  users.forEach((row) => {
    if (row.createdById) actorIds.add(Number(row.createdById));
  });
  ...
  const actorMap = new Map(
    actorRows.map((actor) => [Number(actor.id), actor]),
  );
  ...
  const linkedStudents = studentIds.length
    ? await Student.findAll({ where: { id: studentIds } })
    : [];
  const linkedStudentMap = new Map(
    linkedStudents.map((student) => [Number(student.id), student]),
  );
  const parentIds = [
    ...new Set(parentLinks.map((link) => Number(link.parentId))),
  ];
  const linkedParents = parentIds.length
    ? await Parent.findAll({ where: { id: parentIds } })
    : [];
  const linkedParentMap = new Map(
    linkedParents.map((parent) => [Number(parent.id), parent]),
  );
  ...
  for (const row of users.filter(
    ...
    activities.push({
      ...
      occurredAt: row.getDataValue("createdAt") as Date,
      category: "account",
      href:
        String(row.role).toUpperCase() === "TEACHER"
          ? "/admin/teachers"
  ...
  for (const row of gradeItems) {
    ...
    activities.push({
      ...
      role: roleLabel(actor?.role || "teacher"),
      module: "Grade Publishing",
      occurredAt: row.getDataValue("updatedAt") as Date,
      category: "grades",
      href: "/admin/settings",
    });
  }
...
export async function getPendingTasks() {
  const [
    ...
    TeacherLeaveRequest.count({ where: { status: "PENDING" } }),
    TeacherLeaveRequest.findAll({
      where: { status: "APPROVED" },
      attributes: ["id"],
    }),
    ClassTakeover.count({ where: { status: "ACTIVE" } }),
    Teacher.findAll({
  ...
  const teachersWithoutClasses = teachers.filter(
    (teacher) => !assignedTeacherIds.has(Number(teacher.id)),
  ).length;
  const incompleteClasses = classes.filter(
    (row) => !row.subjectId || !row.sectionId,
  ).length;
  const studentsWithoutParents = Math.max(0, students - linkedStudentCount);
  const missingGradeSubmissions = Number(
    gradeProgress?.totals?.missingClasses ?? 0,
  );
