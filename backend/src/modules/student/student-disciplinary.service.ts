...
import { StudentDisciplinaryRecord } from "../../db/models/StudentDisciplinaryRecord.model";
...
export type DisciplinaryContext = {
  userId: number;
  role: string;
...
function serialize(record: StudentDisciplinaryRecord, creatorName?: string) {
  return {
    id: Number(record.id),
...
async function audit(
  ...
  action: string,
  record: StudentDisciplinaryRecord,
...
export async function listStudentDisciplinaryRecords(
  studentId: number,
  filters: {
    academicYear?: string;
    status?: string;
  ...
  if (!(await Student.findByPk(studentId))) return null;
  ...
  const { rows, count } = await StudentDisciplinaryRecord.findAndCountAll({
    where,
  ...
  const users = creatorIds.length
    ? await User.findAll({ where: { id: { [Op.in]: creatorIds } } })
    : [];
  ...
  const all = await StudentDisciplinaryRecord.findAll({
    where: { studentId },
...
function validate(input: DisciplinaryInput, partial = false) {
  const required = ["incidentDate", "incidentType", "severity", "title", "description"] as const;
  if (!partial && required.some((key) => !cleanText(input[key], 4000)))
    return "Complete all required disciplinary record fields.";
  if (input.severity && !SEVERITIES.has(input.severity)) return "Invalid severity.";
...
export async function createStudentDisciplinaryRecord(
  studentId: number,
  input: DisciplinaryInput,
  context: DisciplinaryContext,
) {
  if (!(await Student.findByPk(studentId)))
    return { ok: false as const, code: 404, message: "Student not found." };
  const academicYear = await currentAcademicYear();
  if (!academicYear)
    return { ok: false as const, code: 409, message: "Set the current Academic Year before adding a record." };
  const error = validate(input);
  if (error) return { ok: false as const, code: 400, message: error };
  const record = await StudentDisciplinaryRecord.create({
    studentId,
    academicYear,
  ...
  return { ok: true as const, record: serialize(record) };
}

export async function updateStudentDisciplinaryRecord(
  studentId: number,
  recordId: number,
  input: DisciplinaryInput,
  context: DisciplinaryContext,
) {
  const record = await StudentDisciplinaryRecord.findOne({ where: { id: recordId, studentId } });
  if (!record) return { ok: false as const, code: 404, message: "Disciplinary record not found." };
  const academicYear = await currentAcademicYear();
  if (!academicYear || record.academicYear !== academicYear)
    return { ok: false as const, code: 409, message: "Historical disciplinary records are read-only." };
  ...
  await record.update({
    ...
    actionTaken:
      input.actionTaken === undefined ? record.actionTaken : cleanText(input.actionTaken, 5000) || null,
    resolutionNotes:
